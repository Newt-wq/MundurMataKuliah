  ...
  if (!pengajuan) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-5 text-center my-12 space-y-3 max-w-md mx-auto">
        ...
        <p className="text-xs">
          Maaf, berkas pengajuan dengan ID <span className="font-mono">{id}</span> tidak terdaftar di sistem.
        </p>
        <Link
          href="/"
          className="inline-block bg-red-800 hover:bg-red-900 text-white font-semibold text-xs py-2 px-4 rounded transition-colors"
        >
  ...
  if (currentUser.role === "mahasiswa" && pengajuan.nim !== currentUser.nim) {
    return (
      <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-5 text-center my-12 space-y-3 max-w-md mx-auto">
        ...
        <p className="text-xs">
          Anda tidak memiliki izin untuk melihat pengajuan mahasiswa lain.
        </p>
        <Link
          href="/"
          className="inline-block bg-amber-800 hover:bg-amber-900 text-white font-semibold text-xs py-2 px-4 rounded transition-colors"
        >
          Kembali ke Dashboard
        </Link>
      </div>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-4 gap-3 print:hidden">
        <div className="space-y-1">
          <Link
            href={currentUser.role === "admin" ? "/admin/pengajuan" : "/riwayat"}
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            title="Unduh file .pdf langsung ke folder download"
          ...
          <Link
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 bg-primary-madani hover:bg-primary-madani-dark text-white font-semibold py-2 px-3.5 rounded-xl text-xs transition-colors cursor-pointer shadow-xs"
          >
      ...
      {/* Admin Comment Notification if Rejected/Approved */}
      {pengajuan.catatanAdmin && (
        <div className={`p-4 rounded-xl border text-xs leading-relaxed print:hidden flex gap-3 ${
          ...
          <div>
            <span className="font-bold">Catatan dari Admin UPPS FIR:</span>
            <p className="mt-1 font-medium italic">"{pengajuan.catatanAdmin}"</p>
          </div>
        </div>
      )}

      {/* EXACT PHYSICAL FORM PRINTABLE CANVAS (Standard Hex Color Inline Styles to avoid html2canvas lab() color parsing crash) */}
      <div 
        id="printable-form"
